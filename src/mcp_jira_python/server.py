    ...
    try:
        ...
        if name == "add_comment":
            ...
            with NamedTemporaryFile(delete=False) as temp_file:
                ...
                with open(temp_file.name, "rb") as f:
                    attachment = jira.add_attachment(
                        issue=issue_key, attachment=f, filename=attachment["filename"]
                    )
            ...
            if not all([inward_issue, outward_issue, link_type]):
                raise ValueError(
                    "inwardIssueKey, outwardIssueKey, and linkType are required"
                )
            ...
            return [
                types.TextContent(
                    type="text", text='{"message": "Issue link created successfully"}'
                )
            ]
        ...
        return [
            types.TextContent(
                type="text", text=f"Operation failed: {str(e)}", isError=True
            )
        ]
